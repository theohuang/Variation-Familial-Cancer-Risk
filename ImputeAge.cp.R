...
ImputeAge.cp = function(fff, params, model, net = FALSE) {
  ...
  if (model=="MMRpro"){cnslage <- max(fff[which(fff$Relation==1),c("AgeColon", "AgeEndometrium")])}
  ...
  if (model=="MMRpro") {ca1 = "AffectedColon"; ca2 = "AffectedEndometrium"; age1 = "AgeColon"; age2 = "AgeEndometrium"; nc="M000"}
  ...
  if (model=="brcapro" | model=="MMRpro") {cnsloc  = fff[which(fff$ID == cnsl),ca2]}
  ...
  # 
  # if (model=='brcapro' | model=='MMRpro') {combi$uua = combi[,ca1]==0 & combi[,ca2]==0 & fff[,age1]==1 & fff[,age2]==1}
  ...
  ##########################################################
  # Scenario 3: BC and OC (brcapro) or CRC and EC (mmrpro) #  
  ...
  if (model=='MMRpro'){
    ifamily = list(fff, mem_bc, mem_oc, age_bc, age_oc)
    names(ifamily) = c('fff', 'mem_bc', 'mem_oc',  'age_bc', 'age_oc')
  }
  if (model=='pancpro' | model=='melapro') {

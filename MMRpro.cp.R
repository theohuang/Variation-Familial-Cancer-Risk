## MMRpro carrier probabilites for whole family

## version 2.1-5

MMRpro.cp <- function(family, counselee.id=1, race = "Unknown", germline.testing=NULL, 
  ...
  if(max(family[which(family$ID == counselee.id), c("AgeColon", "AgeEndometrium")], na.rm = TRUE) > age.max - 1){
    pro.agemax <- 1
  }
  ...
  family <- CheckFamStructure("MMRpro",fff=family, counselee.id=counselee.id, germline.testing=germline.testing, marker.testing=marker.testing, 
  ...
  ages.cancer <- data.frame(family[,"AgeColon",drop=FALSE],family[,"AgeEndometrium",drop=FALSE])
                              ...
                              marker.testing, psize, counselee.id, allef)
  {
    ...
    # dyn.load('peelingC')
    post <- peelingRC(allef=allef,LIK,ped=ped,counselee.id=ped$ID,nloci=nloci)
  ...
  if (missingage & imputeAges==TRUE){
    ...
    # who had age imputed
    if (length(ifamily$mem_bc)>0 | length(ifamily$mem_oc)>0) {
      for (iIter in 1:nIter){
        ...
        postList[,,iIter] = runPeelingMMRpro(family=nf, 
                                             params=params, ngen=ngen,
                                             germline.testing=germline.testing,
                                             marker.testing=marker.testing, psize=psize, 
                                             counselee.id=counselee.id, allef=allef)
      ...
      post = base::rowMeans(postList, dims = 2)
    }

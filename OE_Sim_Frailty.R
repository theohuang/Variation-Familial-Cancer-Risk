...
for(k in 1:nboot){
  ...
  for(i in 1:length(w.list)){
    ## frailty only for colorectal cancer
    ...
    # # frailty-adjusted penetrance
    # res.exp.w <- expect(fam, CP$cancerMDens[1:94, , "ColorC"], CP$cancerFDens[1:94, , "ColorC"])
    # res.obs.w <- observe(fam, CP$cancerMDens[1:94, , "ColorC"], CP$cancerFDens[1:94, , "ColorC"])
    ...
    for(j in 1:nrow(fam)){
      ...
      # fam.o.w[j] <- res.obs.w[j, geno[j]]
      # fam.e.w[j] <- res.exp.w[j, geno[j]]
    }
    ...
    # ## frailty penetrance
    # res.oe[[k]]$O.C.w[i] <- sum(fam.o.w[geno != 1])
    # res.oe[[k]]$E.C.w[i] <- sum(fam.e.w[geno != 1])
    # res.oe[[k]]$O.NC.w[i] <- sum(fam.o.w[geno == 1])
    # res.oe[[k]]$E.NC.w[i] <- sum(fam.e.w[geno == 1])
    # res.oe[[k]]$OE.C.w[i] <- res.oe[[k]]$O.C.w[i] / res.oe[[k]]$E.C.w[i]
    # res.oe[[k]]$OE.NC.w[i] <- res.oe[[k]]$O.NC.w[i] / res.oe[[k]]$E.NC.w[i]
    
    ## using carrier probabilities from the baseline penetrance
    # with censoring
    ...
    # ## using carrier probabilities from the frailty penetrance
    # LIK.w <- estLik(fam, CP, ODP)
    # probs.w <- setNames(data.frame(matrix(0, nrow(fam), ncol(CP$cancerFDens[, , 1]))),
    #                     colnames(CP$cancerFDens[, , 1]))
    # for(j in 1:nrow(fam)){
    #   probs.w[j, ] <- pp.peelingParing(fam, af, LIK.w, length(mutations),
    #                                    counselee.id = fam$ID[j])
    # }
    
    # res.oe[[k]]$O.C.cpw[i] <- sum(res.obs.w[, -1] * probs.w[, -1])
    # res.oe[[k]]$E.C.cpw[i] <- sum(res.exp.w[, -1] * probs.w[, -1])
    # res.oe[[k]]$O.NC.cpw[i] <- sum(res.obs.w[, 1] * probs.w[, 1])
    # res.oe[[k]]$E.NC.cpw[i] <- sum(res.exp.w[, 1] * probs.w[, 1])
    # res.oe[[k]]$OE.C.cpw[i] <- res.oe[[k]]$O.C.cpw[i] / res.oe[[k]]$E.C.cpw[i]
    # res.oe[[k]]$OE.NC.cpw[i] <- res.oe[[k]]$O.NC.cpw[i] / res.oe[[k]]$E.NC.cpw[i]
  }
}
difftime(Sys.time(), start, units = "secs")


save(res.oe, fams, file = paste(getwd(), "/Extended Frailty/OE Simulation/oe_sim_", a1, ".RData", sep = ""))

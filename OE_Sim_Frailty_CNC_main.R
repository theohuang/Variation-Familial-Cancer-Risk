...
for(k in 1:nfam){
  ...
  hzdF <- hzd0F; hzdF$ColorC <- hzd02hzd(hzd0F$ColorC, w = res.oe.sim$W[k])
  hzdM <- hzd0M; hzdM$ColorC <- hzd02hzd(hzd0M$ColorC, w = res.oe.sim$W[k])
...
save(res.oe.sim, res.oe.sim.boot, fams, boot.w, res.sim.hu.c, res.sim.hu.nc,
     res.sim.diff.hu.c, res.sim.diff.hu.nc, file = "OE_Sim_CNC_Main.RData")
